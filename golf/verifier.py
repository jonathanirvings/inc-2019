...
def main(argv):
  ...
  a = vr.ReadInt(0, 1000000000)
  vr.ReadSpace()
  
  b = vr.ReadInt(0, 1000000000)
  vr.ReadSpace()
  
  s = vr.ReadInt(0, 1000000000)
  vr.ReadSpace()
  
  t = vr.ReadInt(1, 1000000000)
  vr.ReadEndl()
  
  assert a < s and s < t and t < b, ('Wrong order')
  
  P = vr.ReadIntList(N, a + 1, b - 1)
  before = a
  for p_i in P:
    assert before < p_i, 'P must be bigger than the previous number'
    assert p_i != s and p_i != t, 'P must be different than s and t'
    before = p_i
  
  H = vr.ReadIntList(N, 1, 1000000000)
